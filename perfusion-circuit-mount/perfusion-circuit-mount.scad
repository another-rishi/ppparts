...
if (assembly) {
    ...
    translate([L-hh-wall_thickness/2,0,bubble_trap_height]) union() {
        translate([0, 75, 0]) bubble_trap_connector();
        translate([0, 75, -(body_height-(body_width+2*d))]) bubble_trap_connector();
        translate([0, -75, 0]) bubble_trap_connector();
        translate([0, -75, -(body_height-(body_width+2*d))]) bubble_trap_connector();
    }
    translate([body_width/2+5*d+wall_thickness/2+(2*L-2*hh-1),75,bubble_trap_height-(5*d+(body_width+2*d)/2)-(body_height-(body_width+2*d))]) xflip() color("orange") bubble_trap();translate([body_width/2+5*d+wall_thickness/2+(2*L-2*hh-1),-75,bubble_trap_height-(5*d+(body_width+2*d)/2)-(body_height-(body_width+2*d))]) xflip() color("orange") bubble_trap();

    translate([L-hh-wall_thickness/2,0,filter_height]) filter_holder_connector();
    translate([L-hh-wall_thickness/2+big+(ro+f+nozzle*4+get_metric_socket_cap_diam(6)+2*nozzle),0,filter_height+4+z/2]) color("orange") outlet();
